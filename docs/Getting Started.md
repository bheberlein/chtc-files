...
The submit file provides detailed information about the job & its requirements. It includes HTCondor needs to know in order to match the job with candidate machines capable of running it, and to start the job on a remote worker node. For example, the submit file can be used to
