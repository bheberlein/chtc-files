...
The reflectance images will remain on Farnsworth when running the BRDF corrections. Because Krusty has a direct mount (at `/mnt/farnsworth`), the imagery can be directly read from & written to the processed data directory on Farnworth (`data/processed/airborne`).
...
**Or**, if the `--grouped-by-site` option is passed, an additional directory level will be added above the session level to organized files by site:
...
4. - **NOTE:** Bad images will be withheld during **fitting** of the BRDF correction, but the correction will be **applied** to all of the images.

5. **Bad images** (i.e. > 10% cloud shadow) **should be removed** from the JSON file.
...
6. **Save modified lines dictionary.**

7. Place lines dictionary JSON in the data directory (e.g. `/$PROJECT`).
