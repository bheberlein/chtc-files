...
resolve_config () {
  ...
  if [ -z ${CONFIG+x} ]; then
    ...
    # First look for flightline-level config
    if [[ -f $CONFIG_DIR/${FLIGHTLINE_CONFIG} ]]; then
    ...
    # Finally, if a separate project basename is given, check for project-level config
    elif [[ ( -n "${PROJECT+x}" ) && ( -f $CONFIG_DIR/${PROJECT_CONFIG} ) ]]; then
